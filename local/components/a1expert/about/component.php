...
if($arParams["ELEMENT_ID"] > 0 && $arParams["ELEMENT_ID"]."" != $arParams["~ELEMENT_ID"])
{
	...
	{
		Iblock\Component\Tools::process404(trim($arParams["MESSAGE_404"]) ?: GetMessage("T_NEWS_DETAIL_NF")
			,true
...
{
	$arNavParams = array("nPageSize" => 1, "bShowAll" => $arParams["PAGER_SHOW_ALL"], );
...
if($arParams["SHOW_WORKFLOW"] || $this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()),$bUSER_HAVE_ACCESS, $arNavigation, $pagerParameters)))
{
	...
	if($arParams["ELEMENT_ID"] <= 0)
		$arParams["ELEMENT_ID"] = CIBlockFindTools::GetElementID($arParams["ELEMENT_ID"], $arParams["~ELEMENT_CODE"], $arParams["STRICT_SECTION_CHECK"]? $arParams["SECTION_ID"]: false, $arParams["STRICT_SECTION_CHECK"]? $arParams["~SECTION_CODE"]: false, $arFilter
		);
	...
	$arSelect = array_merge($arParams["FIELD_CODE"], array("ID", "NAME", "IBLOCK_ID", "IBLOCK_SECTION_ID", "DETAIL_TEXT", "DETAIL_TEXT_TYPE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "TIMESTAMP_X", "ACTIVE_FROM", "LIST_PAGE_URL", "DETAIL_PAGE_URL", ));
	...
	if($obElement = $rsElement->GetNextElement())
	{
		...
		else
		{
			...
			$arResult["NAV_STRING"] = $arResult["NAV_RESULT"]->GetPageNavStringEx($navComponentObject, $arParams["PAGER_TITLE"], $arParams["PAGER_TEMPLATE"], $arParams["PAGER_SHOW_ALWAYS"], $this, $navComponentParameters
			);
		...
		Iblock\Component\Tools::getFieldImageData($arResult, array('PREVIEW_PICTURE', 'DETAIL_PICTURE'), Iblock\Component\Tools::IPROPERTY_ENTITY_ELEMENT, 'IPROPERTY_VALUES'
		);
		...
		foreach($arParams["PROPERTY_CODE"] as $pid)
		{
			...
			if((is_array($prop["VALUE"]) && count($prop["VALUE"])>0) || (!is_array($prop["VALUE"]) && strlen($prop["VALUE"])>0))
			{
				...
				/**
				 * 
				 * Изменения внесенные в компоненt
				 */
				#region Изменения внесенные в компонент
				if($arResult["DISPLAY_PROPERTIES"][$pid]["PROPERTY_TYPE"] == "E")
				{
					/**return array("propsIblockId"=>$propsIblockId, "props"=>$props); */
					$propsTypeE = $fixer->GetTypeE($arResult["DISPLAY_PROPERTIES"][$pid]["LINK_IBLOCK_ID"], $arResult["DISPLAY_PROPERTIES"][$pid]["LINK_ELEMENT_VALUE"]);
					$filter = $fixer->SetFilter(array("IBLOCK_ID"=>$propsTypeE["propsIblockId"], "ID"=>$arResult["DISPLAY_PROPERTIES"][$pid]["VALUE"]));
					$select = $fixer->SetSelect(array("NAME", "DETAIL_TEXT", "PREVIEW_TEXT", "PREVIEW_PICTURE", "DETAIL_PAGE_URL"), $propsTypeE["props"]);
					$result = $fixer->GetElements(array(), $filter, false, false, $select);
					$arResult["DISPLAY_PROPERTIES"][$pid]["RESULT"] = $result;
				}
				if($pid == "GALLERY")
				{
					if(count($arResult["DISPLAY_PROPERTIES"][$pid]["VALUE"]) == 1)
					{// Это нужно потому-что разработчики битрикса ДОЛБАЁБЫ ЖОПОРУКИЕ
						$arResult["DISPLAY_PROPERTIES"][$pid]["VALUE"] = array($arResult["DISPLAY_PROPERTIES"][$pid]["VALUE"]);
						$arResult["DISPLAY_PROPERTIES"][$pid]["FILE_VALUE"] = array($arResult["DISPLAY_PROPERTIES"][$pid]["FILE_VALUE"]);
					}
					foreach ($arResult["DISPLAY_PROPERTIES"][$pid]["FILE_VALUE"] as $k=>$v)
					{
						if(in_array($v["CONTENT_TYPE"], array("video/mpg", "video/avi", "video/wmv", "video/mpeg", "video/mpe", "video/flv")))
						{
							$arResult["DISPLAY_PROPERTIES"][$pid]["VIDEOS"][] = $v;
							unset($arResult["DISPLAY_PROPERTIES"][$pid]["VALUE"][$k]);
						}
					}
					$arResult["DISPLAY_PROPERTIES"][$pid]["RESULT"] = $fixer->MakeGallery($arResult["DISPLAY_PROPERTIES"][$pid]["VALUE"], array("width"=>1000, "height"=>565), array("width"=>360, "height"=>250));
				}
				#endregion Изменения внесенные в компонент
			}
		}
		...
		{
			$rsPath = CIBlockSection::GetNavChain($arResult["IBLOCK_ID"], $arResult["IBLOCK_SECTION_ID"], array("ID", "CODE", "XML_ID", "EXTERNAL_ID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "SORT", "NAME", "ACTIVE", "DEPTH_LEVEL", "SECTION_PAGE_URL"
				)
			);
		...
		$resultCacheKeys = array("ID", "IBLOCK_ID", "NAV_CACHED_DATA", "NAME", "IBLOCK_SECTION_ID", "IBLOCK", "LIST_PAGE_URL", "~LIST_PAGE_URL", "SECTION_URL", "CANONICAL_PAGE_URL", "SECTION", "IPROPERTY_VALUES", "TIMESTAMP_X", );

		if ($arParams["SET_TITLE"] || $arParams["ADD_ELEMENT_CHAIN"] || $arParams["SET_BROWSER_TITLE"] === 'Y' || $arParams["SET_META_KEYWORDS"] === 'Y' || $arParams["SET_META_DESCRIPTION"] === 'Y')
		{
			...
			{
				$arResult["META_TAGS"]["TITLE"] = ($arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"] != "" ? $arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"] : $arResult["NAME"]);
			}

			if ($arParams["ADD_ELEMENT_CHAIN"])
			{
				$arResult["META_TAGS"]["ELEMENT_CHAIN"] = ($arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"] != "" ? $arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"] : $arResult["NAME"]);
			}

			if ($arParams["SET_BROWSER_TITLE"] === 'Y')
			{
				$browserTitle = \Bitrix\Main\Type\Collection::firstNotEmpty($arResult["PROPERTIES"], array($arParams["BROWSER_TITLE"], "VALUE"), $arResult, $arParams["BROWSER_TITLE"], $arResult["IPROPERTY_VALUES"], "ELEMENT_META_TITLE");
				$arResult["META_TAGS"]["BROWSER_TITLE"] = (is_array($browserTitle) ? implode(" ", $browserTitle) : $browserTitle);
				unset($browserTitle);
			}
			if ($arParams["SET_META_KEYWORDS"] === 'Y')
			{
				$metaKeywords = \Bitrix\Main\Type\Collection::firstNotEmpty($arResult["PROPERTIES"], array($arParams["META_KEYWORDS"], "VALUE")
					,$arResult["IPROPERTY_VALUES"], "ELEMENT_META_KEYWORDS"
				);
				$arResult["META_TAGS"]["KEYWORDS"] = (is_array($metaKeywords) ? implode(" ", $metaKeywords) : $metaKeywords);
				unset($metaKeywords);
			}
			if ($arParams["SET_META_DESCRIPTION"] === 'Y')
			{
				$metaDescription = \Bitrix\Main\Type\Collection::firstNotEmpty($arResult["PROPERTIES"], array($arParams["META_DESCRIPTION"], "VALUE"), $arResult["IPROPERTY_VALUES"], "ELEMENT_META_DESCRIPTION");
				$arResult["META_TAGS"]["DESCRIPTION"] = (is_array($metaDescription) ? implode(" ", $metaDescription) : $metaDescription);
	...
	{
		$this->abortResultCache();
		Iblock\Component\Tools::process404(trim($arParams["MESSAGE_404"]) ?: GetMessage("T_NEWS_DETAIL_NF"), true, $arParams["SET_STATUS_404"] === "Y", $arParams["SHOW_404"] === "Y", $arParams["FILE_404"]);
	}
}
...
if(isset($arResult["ID"]))
{
	...
	if(Loader::includeModule("iblock"))
	{
		...
		if($USER->IsAuthorized())
		{
			...
			{
				$arReturnUrl = array("add_element" => CIBlock::GetArrayByID($arResult["IBLOCK_ID"], "DETAIL_PAGE_URL"), "delete_element" => (empty($arResult["SECTION_URL"])?
						$arResult["LIST_PAGE_URL"]:
						$arResult["SECTION_URL"]
					), );

				$arButtons = CIBlock::GetPanelButtons($arResult["IBLOCK_ID"], $arResult["ID"], $arResult["IBLOCK_SECTION_ID"], Array("RETURN_URL" => $arReturnUrl, "SECTION_BUTTONS" => false, ));

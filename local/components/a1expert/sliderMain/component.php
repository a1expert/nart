...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	while($obElement = $rsElement->GetNextElement())
	{
		...
		foreach($arParams["PROPERTY_CODE"] as $pid)
		{
			...
			/**
			 * Изсенения в компоненте. Получает элемент из св-ва "ссылка на элемент" в норм виде, со всеми своими св-вами.
			 */
			if($arItem["DISPLAY_PROPERTIES"][$pid]["PROPERTY_TYPE"] == "E")
			{
				$filter = $fixer->SetFilter(array("IBLOCK_ID"=>$arItem["PROPERTIES"][$pid]["LINK_IBLOCK_ID"], "ID"=>$arItem["PROPERTIES"][$pid]["VALUE"]));
				$select = $fixer->SetSelect(array("DETAIL_PAGE_URL"));
				$result = $fixer->GetElements(array(), $filter, false, false, $select);
				$arItem["DISPLAY_PROPERTIES"][$pid]["RESULT"] = $result;
			}
		}

...
(function (){
	const cardTemplate = document.querySelector('#rentItem').content.querySelector('.rentItem');
	const rentGrid = document.querySelector('.rentGrid .row');
	window.createCard = function (array) {
		const fragment = document.createDocumentFragment();
		array.forEach(function (it) {
			const rentCard = cardTemplate.cloneNode(true);
			...
			rentCardImage.src = 'http://a1reklamaj.myjino.ru' + it.image;
